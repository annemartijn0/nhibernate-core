...
namespace NHibernate.AdoNet
{
	...
	public abstract class AbstractBatcher : IBatcher
	{
		...
		protected readonly HashSet<DbDataReader> _readersToClose = new HashSet<DbDataReader>();
		protected readonly IDictionary<DbDataReader, Stopwatch> _readersDuration = new Dictionary<DbDataReader, Stopwatch>();
		...
		protected void Prepare(DbCommand cmd)
		{
			try
			{
				DbConnection sessionConnection = _connectionManager.GetConnection();
		...
		public void CloseCommands()
		{
			...
			try
			{
				foreach (DbDataReader reader in new HashSet<DbDataReader>(_readersToClose))
				{
					try
					{
						CloseReader(reader);
				...
				foreach (DbCommand cmd in _commandsToClose)
				{
					try
					{
						CloseCommand(cmd);
