	...
	public abstract class DriverBase : IDriver, ISqlParameterFormatter
	{
		...
		private void SetCommandParameters(DbCommand cmd, SqlType[] sqlTypes)
		{
			for (int i = 0; i < sqlTypes.Length; i++)
			{
				...
				DbParameter dbParam = GenerateParameter(cmd, paramName, sqlTypes[i]);
		...
		public virtual void ExpandQueryParameters(DbCommand cmd, SqlString sqlString)
		{
			...
			foreach (object part in sqlString)
			{
				...
				if (parameter != null)
				{
					var originalParameter = (DbParameter)cmd.Parameters[parameter.ParameterPosition.Value];
		...
		{
			DbParameter param = GenerateParameter(command, "ReturnValue", SqlTypeFactory.Int32);
