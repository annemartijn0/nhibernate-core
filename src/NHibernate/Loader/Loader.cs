	...
	public abstract class Loader
	{
		...
		private Task<IList> DoQueryAndInitializeNonLazyCollectionsAsync(ISessionImplementor session, QueryParameters queryParameters, bool returnProxies)
		{
			...
			try
			{
				try
				{
					result = DoQueryAsync(session, queryParameters, returnProxies);
				}
				finally
				{
					persistenceContext.AfterLoad();
		...
		private Task<IList> ListUsingQueryCacheAsync(ISessionImplementor session, QueryParameters queryParameters, ISet<string> querySpaces, IType[] resultTypes)
		{
			...
			var listTask = Task.Factory.StartNew(() =>
				GetResultFromQueryCache(session, queryParameters, querySpaces, resultTypes, queryCache,
				key));

			if (listTask.Result == null)
			{
				listTask = DoListAsync(session, queryParameters);
		...
		protected Task<IList> DoListAsync(ISessionImplementor session, QueryParameters queryParameters)
		{
			...
			Task<IList> result = null;
			try
			{
				result = DoQueryAndInitializeNonLazyCollectionsAsync(session, queryParameters, true);
			}
			...
			result.ContinueWith(task =>
			{
				...
				{
					stopWatch.Stop();
					Factory.StatisticsImplementor.QueryExecuted(QueryIdentifier, task.Result.Count, stopWatch.Elapsed);
				}
			});
