...
namespace NHibernate.Test.SqlTest
{
	...
	public class MonetaryAmountUserType : IUserType
	{
		...
		public object NullSafeGet(DbDataReader resultSet,
		                          string[] names,
		                          object owner)
		{
		...
		public void NullSafeSet(DbCommand statement,
		                        object value,
		                        int index)
		{
			if (value == null)
			{
				((DbParameter) statement.Parameters[index]).Value = DBNull.Value;
				((DbParameter) statement.Parameters[index + 1]).Value = DBNull.Value;
			}
			else
			{
				MonetaryAmount currency = (MonetaryAmount) value;
				((DbParameter) statement.Parameters[index]).Value = currency.Value;
				((DbParameter) statement.Parameters[index + 1]).Value = currency.Currency;
			}
		}
